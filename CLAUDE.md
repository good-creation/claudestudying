...
easy/                       同じ入門コースの「やさしい版」— 言葉を書き直した別ページ群
  index.html                コース概要 + 読み方 + 目次 + やさしい用語集 + この版についての断り書き
  NN-<slug>.html            レッスンページ（01〜11・くわしい版とファイル名をそろえてある）
...
### 同じコースの「やさしい版」を足すとき（`easy/` が先例）

`easy/` は**新しいコースではない**。ルート直下の `NN-*.html`（Claude Code 入門コース）と
**同じ11レッスンを、高校生でも読める言葉に書き直したもの**。出典も扱う話題も結論も同じで、
変えているのは言い回しだけ。だから別コースではなく「同じコースの別の版」として扱う。

- **ファイル名はくわしい版とそろえる**（`easy/05-context-management.html` ⇄ `05-context-management.html`）。
  対応が1対1だと分かるので、書き換えるときに突き合わせやすい
- **各ページから、対応するくわしい版へのリンクを必ず3か所に置く** — ヒーローの `.meta` の
  `くわしい版`、末尾の `.tip`「もっとくわしく知りたいときは」、フッタ右のリンク。
  やさしい版だけ読んで終わりにさせないための導線なので、減らさない
- **タイムラインは持たない**（`data-duration` なし）。`.sec__t` には時刻ではなく短い英字ラベル
  （`WHAT` `DIFF` `AGENT` `RECAP` など）を入れる。`advanced/` のテキスト教材由来のページと同じ扱い
- **原文トランスクリプト（`.tr`）は載せない。** 英語の原文はくわしい版の役割
- 文体は**ですます調**。くわしい版の常体と意図的に変えてある（読み手が変わったことが一目で分かる）
- 専門用語は**必ずその場で言いかえてから使う**。`.tip` の `<b>たとえるなら</b>` が言いかえの定位置
- **公式の「やさしい版」ではない。** `easy/index.html` の `#source` に、これが当サイトによる
  書き直しであることを `.warn` で明記してある。ここは消さない
- ルート側からの導線は3箇所: `tools/rebuild-nav.py` の `EASY`（ルート nav の「やさしい版」ボタン）、
  `index.html` の `.lessons` のカード、`claude-code.html` の下側 `.lessons` の先頭
- `assets/progress.js` の `COURSES` にも `easy` を1系統として足してある（進捗の分母に入る）
- `tools/check-quiz.py` と `tools/shuffle-quiz.py` の対象ファイル一覧にも `easy/*.html` を追加済み。
  **やさしい版に `.chk` を足したら、くわしい版と同じ手順で通す**
